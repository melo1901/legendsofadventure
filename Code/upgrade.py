...
class Upgrade:
    ...
    def input(self):
        ...
        if self.can_move:
            ...
            ) and self.select_index < self.attrib_nr - 1:
                self.select_index += 1
                self.can_move = False
    ...
    def display(self):
        ...
        for index, item in enumerate(self.item_list):
            ...
            item.display(
                self.display_surface, self.select_index, name, cost, amount, max_amount
            )
...
class ShopItem:
    ...
    def display_names(self, surface, name, cost, amount, max_amount, selected):
        ...
        amount_rect = amount_surf.get_rect(
            midtop=self.rect.midtop + pygame.math.Vector2(0, title_rect[1] + 20)
        )
        # cost
    ...
    def trigger(self, player):
        ...
        if upgrade_attrib == "sword" or upgrade_attrib == "axe":
            if (
                ...
                and weapon_data[upgrade_attrib]["damage"]
                < player.upgrade_gold_max_values[upgrade_attrib]
            ):
                weapon_data[upgrade_attrib]["damage"] += player.upgrade_gold_value[
                    upgrade_attrib
                ]
                player.gold -= player.upgrade_gold_cost[upgrade_attrib]
            ...
            if (
                weapon_data[upgrade_attrib]["damage"]
                > player.upgrade_gold_max_values[upgrade_attrib]
            ):
                weapon_data[upgrade_attrib]["damage"] = player.upgrade_gold_max_values[
                    upgrade_attrib
                ]
        elif upgrade_attrib == "heal":
            if (
                player.gold >= player.upgrade_gold_cost[upgrade_attrib]
                and magic_data[upgrade_attrib]["strength"]
                < player.upgrade_gold_max_values[upgrade_attrib]
            ):
                magic_data[upgrade_attrib]["strength"] += player.upgrade_gold_value[
                    upgrade_attrib
                ]
                player.gold -= player.upgrade_gold_cost[upgrade_attrib]
                player.upgrade_gold_cost[upgrade_attrib] += 5
                upgrade_gold_data[upgrade_attrib]["actual"] += 1
            if (
                magic_data[upgrade_attrib]["strength"]
                > player.upgrade_gold_max_values[upgrade_attrib]
            ):
                magic_data[upgrade_attrib]["strength"] = player.upgrade_gold_max_values[
                    upgrade_attrib
                ]
        elif upgrade_attrib == "flame_damage":
            if (
                player.gold >= player.upgrade_gold_cost[upgrade_attrib]
                and magic_data["flame"]["damage"]
                < player.upgrade_gold_max_values[upgrade_attrib]
            ):
                magic_data["flame"]["damage"] += player.upgrade_gold_value[
                    upgrade_attrib
                ]
                player.gold -= player.upgrade_gold_cost[upgrade_attrib]
                player.upgrade_gold_cost[upgrade_attrib] += 5
                upgrade_gold_data[upgrade_attrib]["actual"] += 1
            if (
                magic_data["flame"]["damage"]
                > player.upgrade_gold_max_values[upgrade_attrib]
            ):
                magic_data["flame"]["damage"] = player.upgrade_gold_max_values[
                    upgrade_attrib
                ]
        elif upgrade_attrib == "flame_range":
            if (
                player.gold >= player.upgrade_gold_cost[upgrade_attrib]
                and player.magic_range < player.upgrade_gold_max_values[upgrade_attrib]
            ):
    ...
    def display(self, surface, select_nr, name, cost, amount, max_amount):
        ...
        self.display_names(
            surface, name, cost, amount, max_amount, self.index == select_nr
        )
