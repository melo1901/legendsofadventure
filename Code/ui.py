...
class UI:
    ...
    def show_tmp_bar(self, player, bg_rect, color):
        ...
        if player.health < player.target_health:
            ...
            transition_width = abs(
                int((player.target_health - player.health))
                / player.max_health
                * bg_rect.width
            )
    ...
    def weapon_overlay(self, weapon_index):
        bg_rect = self.selection_box(
            10, (self.display_surface.get_size()[1] - 110)
        )  # weapon
    ...
    def magic_overlay(self, magic_index):
        bg_rect = self.selection_box(
            80, (self.display_surface.get_size()[1] - 90)
        )  # weapon
    ...
    def display(self, player):
        ...
        self.show_bar(
            player.mana, player.stats["mana"], self.stamina_bar_rect, MANA_COLOR
        )
